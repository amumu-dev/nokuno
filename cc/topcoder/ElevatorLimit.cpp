...
public:
  vector <int> getRange(vector <int> enter, vector <int> exit, int physicalLimit) {
    int person = 0, minimum = 0, maximum = 0;
    for (int i = 0; i < (int)enter.size(); i++) {
      person -= exit[i];
      minimum = min(person, minimum);
      person += enter[i];
      maximum = max(person, maximum);
    }
    vector <int> result;
    if (maximum - minimum <= physicalLimit) {
      result.push_back(abs(minimum));
      result.push_back(abs(physicalLimit - maximum));
    }
    return result;
  }
