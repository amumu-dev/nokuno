...
class plsa:
    ...
    def likelihood(self):
        ...
        # Σ n(d,w) log P(d,w)
        return sum([ self.n[d][w]*plsa.mylog(self.pdw[d][w]) for d in range(self.nd) for w in range(self.nw) ])
...
#                self.pdwz[d][w] = self.normalized(self.pdwz[d][w])
                self.pdwz[d][w] = self.maximized(self.pdwz[d][w])
...
#            self.pzd[z] = self.normalized(self.pzd[z])
    ...
    def get_pdz(self):
        return [self.normalized([self.pzd[j][i] * self.pz[j] for j in range(self.nz)]) for i in range(self.nd)]
