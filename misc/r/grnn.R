...
grnn <- function(sigma) {
	...
	m <- function(x) {
		input <- outer(x, curve_fitting$x, kernel)
		regular <- apply(input, MARGIN=1, sum)
		input %*% curve_fitting$t / regular
	}
	plot(m, xlim=c(0,1), ylim=c(-1,1), xlab="", ylab="")
	par(new=T)
	plot(curve_fitting, xlim=c(0,1), ylim=c(-1,1), xlab=paste("sigma=",sigma), ylab="")
}